partial class Part4
{
	public static void variableData2()
	{
					...
					for (int i = 0; i < maxPets; i++)
					{
						if (ourAnimals[i, 1].Contains("dog"))
						{
							...
							foreach (string dogCharacteristic in dogCharacteristics)
							{
								...
								for (int j = 3; j > -1; j--)
								{
									Console.Write($"\rsearching our dog {ourAnimals[i, 3]} for {dogCharacteristic} {searchingIcons[j]} {j}");
									Thread.Sleep(100);
								}
							...
							if (!noMatchesDog) Console.WriteLine(ourAnimals[i, 0] + "\n" + dogDescription + "\n");
						}
					}
